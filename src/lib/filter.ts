// src/lib/filter.js — pure filtering + sorting helpers used by the list view.
...
export function filterTodos(todos: any[], filter: string): any[] {
  switch (filter) {
    case "active":
...
export function sortTodos(todos: any[]): any[] {
  return todos
    .map((t, index) => ({ t, index }))
    .sort((a, b) => {
