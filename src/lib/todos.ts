...
export function createTodo(input: { title?: string; priority?: string }): { id: string; ref: string; title: string; priority: string; done: boolean; createdAt: string } {
  ...
  const priority = PRIORITIES.includes(input && input.priority ? input.priority : "")
    ? (input && input.priority) as string
    : "normal";
  return {
    id: newId(),
    ref: newShortRef(),
